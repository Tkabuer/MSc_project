...
def train(args, train_loader_syn, \
    ...
    for step,((syn_i1, syn_i2, dist_label_syn)) in enumerate(train_loader_syn):
        
        #image 1
        syn_i1 = syn_i1.cuda(non_blocking=True)
        # ugc_i1 = ugc_i1.cuda(non_blocking=True)
        # x_i1 = torch.cat((syn_i1,ugc_i1),dim=0)
        # x_i1.shape = torch.Size([64, 36, 256, 256]) 64是两个32的batch_size合起来了 36是12帧图片RGB三个颜色通道
        #image 2
        ...
        dist_label = torch.zeros((args.batch_size, \
                                  args.clusters))
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            = model(syn_i1, syn_i2)
            # print(len(z_i1_patch))
            # print(len(z_i1_patch[0]))
        ...
        if args.nr == 0:
            np.save(args.model_path + 'LRs.npy', LR)
            args.global_step += 1
...
def main(gpu, args):
    ...
    #sgd optmizer
    args.steps = len(train_loader_syn) #,len(train_loader_ugc))
